...
class TestPostDespesas(TestCase):
    ...
    def test_should_be_create_a_despesa_with_insert_a_categoria_and_verify_data(self):
        ...
        response = self.app.post(self.URL, json=data, headers={'Authorization': 'Bearer '+self.token})
        self.assertEqual(value.get_json(), response.get_json())
        self.assertEqual(response.status_code, 200)
        data_value = self.app.get(self.URL + "/1", headers={'Authorization': 'Bearer '+self.token})
    ...
    def test_should_be_retorn_message_error_by_duplicate_data(self):
        ...
        self.app.post(self.URL, json=data_1, headers={'Authorization': 'Bearer '+self.token})
        response = self.app.post(self.URL, json=data_2, headers={'Authorization': 'Bearer '+self.token})
        self.assertEqual(value.get_json(), response.get_json())
